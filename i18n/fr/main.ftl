...
sync-heading = Suivi de synchronisation
sync-state-idle = Pas encore synchronisé
sync-state-queued = En attente
sync-state-running = Synchronisation en cours
sync-state-ok = Calendrier vérifié
sync-state-failed = Échec de synchronisation
sync-background-note = Cette page se rafraîchit automatiquement. Vous pouvez la fermer : la synchronisation continue en arrière-plan (3 minutes maximum).
sync-stage = Dernière étape atteinte
sync-elapsed = Durée
sync-reference = Référence de diagnostic
sync-last-verified = Dernière vérification complète
sync-never = jamais
sync-back = Retour aux calendriers
sync-setup-write = Choisir le calendrier des nouvelles réservations
sync-failed-note = Le dernier calendrier complet est conservé. Les disponibilités sont suspendues jusqu’à une vérification réussie. Transmettez uniquement la référence, l’étape, la durée et le code d’erreur à votre administrateur.
sync-stage-queued = Attente d’un traitement disponible
sync-stage-credentials = Préparation de l’authentification
sync-stage-discover-principal = Découverte du compte
sync-stage-discover-calendar-home = Recherche de l’emplacement des calendriers
sync-stage-list-calendars = Liste des calendriers
sync-stage-inventory = Liste des versions des événements
sync-stage-download-events = Téléchargement des événements modifiés
sync-stage-verify-calendar-state = Vérification de la cohérence du calendrier
sync-stage-commit-snapshot = Enregistrement du calendrier complet
sync-stage-response-headers = Attente de la réponse du serveur de calendrier
sync-stage-response-body = Réception de la réponse du serveur de calendrier
sync-error-deadline = Le délai maximal de 3 minutes a été dépassé.
sync-error-timeout = Le serveur de calendrier a mis trop de temps à répondre.
sync-error-connect = Connexion au serveur impossible. Vérifier DNS, réseau et TLS.
sync-error-http-status = Le serveur a refusé la requête. 401/403 : vérifier l’authentification ou les permissions ; 5xx : vérifier DavMail/Exchange.
sync-error-invalid-response = Le serveur a renvoyé une réponse CalDAV invalide ou incomplète.
sync-error-remote-changed = Le calendrier a changé pendant la synchronisation. Réessayez.
sync-error-no-calendars = Aucun calendrier exploitable reçu. Vérifier le compte et ses permissions.
sync-error-database = Enregistrement local impossible. Vérifier l’accès à la base et l’espace disque.
sync-error-interrupted = La synchronisation a été interrompue, possiblement par un redémarrage. Réessayez.
sync-error-invalid-calendar = Un événement ne peut pas être lu correctement. Aucun calendrier partiel n’a été publié.
sync-error-invalid-recurrence = Un événement contient une règle de récurrence invalide.
sync-error-unsupported-recurrence = Un format de récurrence ne peut pas encore être traité de façon sûre.
sync-error-unsupported-timezone = Un événement utilise un fuseau horaire non pris en charge.
sync-error-unsupported-duration = Un événement utilise une durée qui ne peut pas être lue correctement.
sync-error-response-too-large = La réponse du serveur dépasse la limite de sécurité de 16 Mio.
sync-error-snapshot-too-large = Le calendrier dépasse la limite de sécurité de 64 Mio.
